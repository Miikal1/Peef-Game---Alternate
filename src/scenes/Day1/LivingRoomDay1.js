class LivingRoomDay1 extends Phaser.Scene {
    constructor() {
        super('livingRoomDay1');
    }
    ...
    update(){
        ...
        if (this.physics.overlap(this.p1, this.doorLeft)){
            this.p1.x = 55;
            this.scene.switch('diningRoomDay1');
        }
        ...
        if (this.physics.overlap(this.p1, this.spool) && Phaser.Input.Keyboard.JustDown(this.keyR) && this.talking == false){
            inventory.push("spool");
            this.spool.destroy();
        } 

        if (this.physics.overlap(this.p1, this.cards) && Phaser.Input.Keyboard.JustDown(this.keyR) && this.talking == false){
